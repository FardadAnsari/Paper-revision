...
[Move here: Initial 2 paragraphs about noise importance and Sun et al.'s limitations]
[Keep Figure \ref{fig:Performance of Models under the various noise power}]

\subsection{Mathematical Modeling of Imperfections}
[Move here: Your AWGN equations:
\begin{equation}
\tilde{S}=S+\textbf{n}
\end{equation}
\begin{equation}
...
[Move here: DNNet description with equations:
\begin{equation}
\left\{
\begin{array}{ll}
	z  & \mbox{if } l=1 \\
...
[Move here: Paragraphs about DNNet's parameter explosion and training complexity]
[Keep all critical observations about FC layers]
...
The model incorporates a denoising network to remove the added noise. This network uses convolutional layers to clean the CSI feedback before it is further processed, ensuring high accuracy even under noisy conditions. In a symmetric manner, the convolution filters are also present in the decoder part of the architecture. Before the refinement step, we reshape the CSI matrix to 2×32×32 (Figure \ref{fig:figure3}).
...
[Move here: Your federated learning approach description]
