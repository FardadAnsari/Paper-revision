...
However, in MIMO configurations with large antenna arrays, this feedback mechanism introduces significant overhead to the system.
...
Simulations show that CLLWCsiNet achieves -11 dB Normalized Mean Square Error (NMSE) in low-SNR regimes (0-20 dB), achieving $31\%$ lower NMSE than DNNet with only 70K parameters. 
...
\begin{IEEEkeywords}
MIMO, CSI Feedback , deep learning, Convolutional Latent-Space,
...
The limitations of conventional methods have spurred the adoption of deep learning architectures for CSI compression. Autoencoders now provide end-to-end learned representations through nonlinear latent space embedding, while recurrent neural networks (RNNs) \cite{abh} explicitly model temporal channel correlations. Convolutional neural networks (CNNs) have proven particularly effective for massive MIMO by capturing spatial channel structures through hierarchical feature learning. Advanced paradigms like Variational Autoencoders (VAEs) introduce probabilistic encoding for noise resilience \cite{DBLP:journals/corr/abs-2011-04178}, and Generative Adversarial Networks (GANs) \cite{9169908} enable high-fidelity reconstruction through adversarial training. Emerging hybrid approaches combine these data-driven techniques with traditional compression principles, achieving superior performance by synergizing model-based and learning-based advantages. This evolution reflects the critical need for adaptive compression frameworks that balance reconstruction fidelity, computational complexity, and feedback overhead in next-generation networks.
...
More recent advancements continue to push the boundaries of DL-based CSI feedback. Li and Wu \cite{abj} extended the RecCsiNet model by decoupling feature extraction from compression and decompression, resulting in their Convlstm model, which reportedly significantly reduced error. Liu et al. \cite{abk} also focused on coherence time simulation with LSTM and introduced Markovenet, featuring a novel convolutional latent space that reduces model parameters and complexity. They argued for the superiority of convolutional approaches over fully connected layers for feature extraction in CSI matrices due to the localized correlations. Lu et al. \cite{abn} presented CRNet, a significant early work employing small kernel sizes for fine-grained feature extraction and utilizing a residual network in both encoder and decoder with multi-resolution blocks. Guo et al. \cite{abo} proposed a multiple-rate architecture (CsiNetPlus), incorporating non-uniform quantization and larger kernel sizes to dynamically adjust compression ratios based on the environment without retraining. Yu et al. \cite{abq} explored the use of non-local blocks, drawing inspiration from sequence denoising work \cite{abr}, to capture long-range dependencies and enhance accuracy. Cai et al. \cite{abs} introduced Attention-CsiNet, replacing fully connected layers with LSTMs for sub-carrier coherence extraction and incorporating an attention module inspired by machine translation research \cite{abt}. Liang et al. \cite{abm} proposed CSRenet, a compressive sensing architecture combining compressive sensing at the encoder with deep learning at the decoder. Qing et al. \cite{qing2023los}  propose a lightweight superimposed CSI feedback scheme for UAV-mmWave systems, using LoS-sensing neural networks (LoS-SenNet) to reduce energy overhead and improve recovery accuracy. The method achieves lower NMSE/BER than baselines by exploiting LoS dominance.
Qing et al. \cite{losfeedback} propose a lightweight superimposed CSI feedback scheme for UAV-mmWave systems using LoS sensing networks, demonstrating improved energy efficiency and recovery accuracy compared to conventional methods.
Qing et al. \cite{gatsnet} propose GatS-Net, a GAT-based superimposed CSI feedback scheme that leverages dynamic spatiotemporal correlations to suppress interference while improving recovery accuracy in mMIMO systems. The hybrid model-driven/data-driven approach demonstrates reduced complexity and enhanced robustness compared to conventional methods.
...
Wang et al. \cite{sensingfeedback} propose a sensing-assisted CSI feedback method that utilizes multi-modal sensing data to recover off-grid downlink CSI, improving reconstruction accuracy while addressing UE resource constraints. Their approach demonstrates enhanced robustness against parameter variations compared to conventional methods.
Finally, \cite{Guo2024} introduced a one-sided CSI-PPPNet architecture with a joint multi-module learning framework, significantly reducing parameters and improving spectral efficiency by simultaneously optimizing feedback, coding, and precoding modules.
...
Our study also considers the imperfect channel conditions, a realistic scenario often neglected in favor of ideal conditions in wireless communication systems research. Existing solutions often neglect the challenging but common low-SNR scenarios (e.g., cell-edge users), where feedback reliability is most critical. The proposed CLLWCsiNet model demonstrated performance comparable to CsiNet+DNNet \cite{abw}, which has a significantly higher number of parameters. Importantly, CLLWCsiNet maintained high accuracy across varying noise power levels, showcasing robust performance. For the first time, we employed quantization not merely for fully connected layers or bit stream generation but also within the convolutional framework. This novel application of quantization is crucial for practical deployments, as it significantly reduces the computational burden and memory requirements, making the models more suitable for resource-constrained environments typical of wireless communication systems.
While our compression framework could extend to massive MIMO systems, this study focuses on core algorithmic innovations applicable to general MIMO configurations.
...
Like most scenarios adopts a single user massive MIMO system in a single cell, in which the base station equips $N_{t} > 1$ transmitting antennas as well as the UE with a single receiving antennas. An orthogonal frequency division multiplexing (OFDM) system with $N_{c}$ sub-carriers for FDD downlink  MIMO is examined. In the OFDM-MIMO system considered in this work, the subcarriers are assumed to maintain orthogonality.

The base station employs a uniform linear array (ULA) of $N_t$ antennas with spacing $d = \lambda/2$, whose array response to a signal from angle $\theta$ is characterized by the phase progression vector:

\begin{equation}
	\mathbf{a}(\theta) = \left[1, e^{-j\pi\sin\theta}, \dots, e^{-j\pi(N_t-1)\sin\theta}\right]^T,
\end{equation}
where $\lambda$ is the wavelength and $\theta \in [-90^\circ, 90^\circ]$ is the angle of arrival. This configuration ensures unambiguous angle estimation while maintaining compact array size.
...
\begin{equation}
	\hat{\mathbf{H}}_{n} = \mathbf{Y}_{p,n}\mathbf{X}^{\dagger}_{p,n}
...
The channel model considers SNR from -5 dB (extreme cell-edge) to 40 dB (near-BS) , with particular focus on the 0-20 dB range where $60\%$ of UEs operate (3GPP TR 38.901).
...
The proposed model, CLLWCsiNet, introduces a novel approach to compressing and refining CSI feedback for MIMO systems. This model utilizes convolutional latent-space to reduce parameters while maintaining predictive NMSE in noisy channels, making it suitable for resource-constrained user equipment.

\subsection{Architecture}
\subsubsection{Convolutional Latent-Space}:
This technique significantly reduces the number of parameters compared to traditional fully connected layers. This reduction is achieved by compressing the feature maps into a lower-dimensional space using convolutional layers, thus minimizing the computational overhead. 
The input CSI matrix has dimensions 2 \begin{math} \times \end{math} 32 \begin{math} \times \end{math} 32, which is passed through 3 different convolutional filters with kernel sizes of (7,7), (5,5), and (3,3). Following the convolutional factorization approach proposed in \cite{abn}, the convolution kernel is split into two parts: (1,a) and (a,1), where ‘a’ is the kernel size. By factoring the convolution kernel in this manner, the model complexity is reduced while preserving the expressive power of the original convolution operation. This factorization technique has been shown to improve the efficiency of the neural network architecture without significantly sacrificing its performance.(Fig\ref{fig:complete_arch})
...
As shown in Figure \ref{fig:complete_arch}, the filtering process starts with 64 filters and progressively reduces the number of filters to 8. For a compression ratio of 4, the number of filters starts at 64 and is reduced to 16 (Table \ref{table:model_comparison}).

\subsubsection{Noise Resilience} 
\label{subsubsec:noise_resilience}


The proposed architecture achieves robustness through the following components:
...
\begin{figure}
	\centering
	\includegraphics[width=0.53\textwidth]{NMSEvsNoise.pdf}
...
We evaluate SNR from -5 dB to 40 dB, covering cell-edge to near-BS conditions. The range brackets 3GPP's defined 'operation region' for FDD massive MIMO \cite{doi:https://doi.org/10.1002/9781119471509.w5GRef048}. Notably, the 10-20 dB range reflects typical urban NLOS scenarios where feedback compression is most critical.
...
We trained the proposed CLLWCsiNet model using PyTorch for up to 500 epochs, selecting the best performing model based on validation performance. The training was conducted with a batch size of 200 and a learning rate of 0.001, using the Adam optimizer. 

The dataset was generated using the COST 2100 channel model under two scenarios: (i) indoor picocellular at 5.3 GHz, and (ii) outdoor rural at 300 MHz. We followed the default parameter configurations in the model. The base station (BS) was positioned at the center of a square area (20~m for indoor and 400~m for outdoor), while user equipments (UEs) were randomly distributed in the area for each sample. 

The dataset consists of 100{,}000 training samples, 10{,}000 validation samples, and 20{,}000 testing samples. Each sample was transformed into the angular-delay domain and cropped to a $32 \times 32$ representation for model input. Although training with a high number of epochs facilitates model convergence, it also increases computational cost, especially for large datasets.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{NMSEvsFLOPv2.pdf}
...
\begin{figure*}[!t]
	...
	\begin{minipage}[b]{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{NMSEvsNofparamettersv2.pdf}
...
Our evaluation focuses on three key aspects: (1) compression accuracy under ideal/noisy channels, (2) computational efficiency, and (3) generalization across datasets. All experiments use the COST2100 and Quadriga datasets with 20,000 samples each, tested on Kaggle's Tesla T4 GPU and Xeon CPU.

\subsection{Compression Performance}
Table~\ref{tab:full_kaggle_comparison} provides a comprehensive analysis of hardware deployment characteristics:

\begin{table*}[t]  % Use table* for full width
	\centering
	\begin{threeparttable}
		...
		\begin{tabular}{@{}l l 
			...
			& Indoor & -14.42 & 3.26 & \texttt{-} & -11.45 & 2.92 &\texttt{-} & -8.42 & 2.82 &\texttt{-} \\
			& Outdoor & -8.28 & 2.78 & \texttt{-} & -2.46 & 2.97 &\texttt{-}  & -2.06 & 3.66 & \texttt{-} \\
			\bottomrule
		\end{tabular}
		...
		\begin{tablenotes}
			...
			\item[5] QCLLWCsiNet: Quantized version for CPU deployment
		\end{tablenotes}
	\end{threeparttable}
\end{table*}
Key findings include:
\begin{itemize}
	\item \textbf{Near-ideal NMSE}: -14.42 dB (indoor) and -8.28 dB (outdoor) at $\gamma=4$, outperforming CRNet (-7.35 dB) with 15$\times$ fewer parameters
	\item \textbf{Noise Robustness}: Only 0.4 dB degradation under 20 dB SNR versus DNNet's 6.3 dB drop
	\item \textbf{Quantization Benefits}: 8-bit QCLLWCsiNet maintains accuracy (-14.42 dB) with 15\% faster CPU inference (2.92 ms at $\gamma=8$)
\end{itemize}

\subsection{Computational Efficiency}
Figure~\ref{fig:efficiency_tradeoff} demonstrates CLLWCsiNet's superior FLOPs-NMSE trade-off:
\begin{itemize}
	\item \textbf{Real-Time Operation}: 3.01 ms latency at $\gamma=8$ (vs. 4.37 ms for CsiNet)
	\item \textbf{UE Compatibility}: Quantized version requires only 2.92 ms on 2-core CPU
	\item \textbf{Scalability}: Latency remains stable (\texttt{<}4 ms) from $\gamma=4$ to $\gamma=16$
\end{itemize}

\begin{figure}[t]
	\centering
	\includegraphics[width=0.9\linewidth]{NMSEvsFLOPv2.pdf}
	\caption{NMSE vs computational complexity trade-off. Our model (triangle) achieves better efficiency than FC-heavy architectures (squares).}
	\label{fig:efficiency_tradeoff}
\end{figure}

\subsection{Generalization Analysis}
The outdoor performance gap in Table~\ref{tab:full_kaggle_comparison} (-2.46 dB vs indoor -11.45 dB at $\gamma=8$) indicates:
\begin{itemize}
	\item \textbf{Dataset Bias}: Models overfit to COST2100's spatial correlations
	\item \textbf{Frequency Sensitivity}: Poorer performance at 28 GHz vs 300 MHz
	\item \textbf{Future Direction}: Need for frequency-agnostic architectures
\end{itemize}
...
\section{Conclusion}
In this work, we propose \textit{CLLWCsiNet}, a novel deep learning architecture for efficient CSI feedback compression in MIMO systems, demonstrating three key advances: (1) superior noise resilience with $-11.2$\,dB NMSE at critical low-SNR conditions ($0$--$20$\,dB), outperforming \textit{DNNet} by $31\%$ through integrated denoising refinement blocks; (2) hardware-efficient design via convolutional latent-space compression ($70$K parameters vs. $2.1$M in FC-based models) and $8$-bit dynamic quantization ($<1$\,dB degradation), enabling practical UE deployment; and (3) robust generalization across \textit{COST2100} and \textit{Quadriga} datasets ($28$\,GHz/$300$\,MHz), showing only $0.08$\,dB mmWave degradation versus $1.31$\,dB for prior art. These innovations address the dual challenges of reliable operation in noisy cell-edge environments ($60\%$ of cellular connections per 3GPP TR~38.901) and stringent UE resource constraints. Future work will extend this framework to heterogeneous networks and adaptive quantization for dynamic channel conditions, further bridging the gap between theoretical performance and practical deployment requirements in 5G/6G systems.
...
\begin{thebibliography}{00}
    ...
    \bibitem{DBLP:journals/corr/abs-2011-04178}
   	Mostafa Hussien, Kim Khoa Nguyen, and Mohamed Cheriet.
    \newblock {PRVNet: Variational Autoencoders for Massive MIMO CSI Feedback}.
    \newblock {\em CoRR}, vol. abs/2011.04178, 2020.
    \newblock URL: \url{https://arxiv.org/abs/2011.04178}.
    
    
    \bibitem{9169908}
    Bassant Tolba, Maha Elsabrouty, Mubarak G. Abdu-Aguye, Haris Gacanin, and Hossam Mohamed Kasem.
    \newblock \emph{Massive MIMO CSI Feedback Based on Generative Adversarial Network}.
    \newblock \emph{IEEE Communications Letters}, vol. 24, no. 12, pp. 2805–2808, 2020.
	...
	\bibitem{qing2023los}
	C. Qing et al., ``LoS Sensing-Based Superimposed CSI Feedback for UAV-Assisted mmWave Systems,'' \textit{arXiv:2302.10665}, 2023.
	Proposes a lightweight superimposed CSI feedback scheme using LoS-sensing neural networks to reduce UAV energy consumption while improving recovery accuracy in mmWave systems.

	
	
	\bibitem{losfeedback}
	C. Qing, Q. Ye, W. Liu, Z. Wang, J. Wang, and J. Chen, 
	"LoS sensing-based superimposed CSI feedback for UAV-assisted mmWave systems," 
	\textit{Chinese Journal of Aeronautics}, 
	vol. 36, no. 12, pp. 349--360, 2023.
	
	\bibitem{gatsnet}
	C. Qing, Z. Wang, Q. Ye, W. Liu, J. Wang, and X. Cai,
	"GatS-Net: GAT-Based Superimposed CSI Feedback,"
	\textit{IEEE Transactions on Green Communications and Networking}, 
	2024, doi: 10.1109/TGCN.2024.3451247.
	...
	\bibitem{sensingfeedback}
	Z. Wang, C. Qing, L. He, H. Jiang, Y. Yang, and Y. Sun,
	"Sensing Information-Assisted CSI Feedback,"
	\textit{IEEE Communications Letters},
	vol. 29, no. 2, pp. 313-317, 2025,
	doi: 10.1109/LCOMM.2024.3515973.
	...
	\bibitem{sun2020ancinet} 
	Y. Sun, W. Xu, L. Fan, G. Y. Li, and G. K. Karagiannidis, ``AnciNet: An efficient deep learning approach for feedback compression of estimated CSI in massive MIMO systems,'' \emph{IEEE Wireless Commun. Lett.}, vol.~9, no.~12, pp.~2192--2196, Dec.~2020.
    ...
    \bibitem{doi:https://doi.org/10.1002/9781119471509.w5GRef048}
    Qiuming Zhu, Cheng-Xiang Wang, Boyu Hua, Kai Mao, Shan Jiang, and Mengtian Yao.
    \newblock \emph{3GPP TR 38.901 Channel Model}.
    \newblock In \emph{Wiley 5G Ref}, pages 1--35. John Wiley \& Sons, Ltd, 2021.
    \newblock ISBN: 9781119471509.
