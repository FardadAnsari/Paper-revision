...
Zhang et al. \cite{zhang2022quantization} addresses the critical challenge of quantization in deep learning-based CSI feedback for massive MIMO systems. The authors propose an innovative adaptor-assisted quantization strategy that significantly improves feedback efficiency while maintaining reconstruction accuracy.
\cite{liu2023continuous} tackles a critical limitation of deep learning (DL)-based CSI feedback in massive MIMO systems: the performance degradation caused by distribution shifts between training and real-world deployment scenarios. The authors propose a continuous online learning framework that addresses catastrophic forgetting while maintaining high feedback accuracy across diverse channel conditions.
\cite{wang2024better} introduces a novel test channel-based quantization module (TCQM) that addresses critical limitations in DL-based CSI feedback systems.
\cite{zhang2024zone} proposes a novel zone-specific deep learning approach for CSI feedback in massive MIMO systems. The key innovation partitions the coverage area into spatial zones with distinct channel characteristics, training specialized autoencoders for each zone.
\cite{chen2024transnet} propose TransNet+, an enhanced Transformer-based architecture for massive MIMO CSI feedback. The method improves upon existing Transformer designs by incorporating higher-dimensional feature representations and an optimized multi-head attention mechanism. A warm-up-aided cosine training scheme is introduced to accelerate convergence and enhance feature learning. Experimental results demonstrate superior performance compared to state-of-the-art methods, particularly at high compression ratios, with significant NMSE reduction in both indoor and outdoor scenarios.
\cite{li2024lightweight}Proposes GCRNet, a Ghost module-based architecture that reduces feature redundancy in CSI feedback by generating secondary features from primary maps. Achieves 22.15\% lower complexity than \cite{ji2021clnet} while maintaining comparable accuracy, addressing efficiency decay in convolution-based methods like \cite{abe}.
\cite{wang2024variational}Proposes a variational autoencoder framework for noisy-channel CSI feedback, formulating it as rate-distortion optimization. The method achieves 40\% bandwidth reduction versus \cite{abe} while maintaining NMSE robustness across SNRs, addressing limitations of uniform bit treatment in \cite{abn,chen2024transnet}.
\cite{mashhadi2020deepcmc}Proposes DeepCMC, a fully convolutional autoencoder with quantization and entropy coding blocks that achieves 3--5~dB lower NMSE than \cite{abe} at equivalent bit rates. Extends to a distributed version exploiting inter-user CSI correlations, reducing feedback overhead by 17--50\% for nearby users 
\begin{math}
	(<1m 
\end{math} 
spacing)
compared to single-user compression.
\cite{guo2023ris}Proposes RIS-CoCsiNet, a cooperative CSI feedback framework for RIS-assisted systems that separates RIS-UE CSI into shared (among nearby UEs) and unique components. Achieves 8.93 dB NMSE improvement over conventional phase feedback at 0.5 BPD through magnitude-dependent phase feedback (MDPF) strategies. The distributed architecture reduces redundant feedback by 17-50\% for UEs within 0.3m spacing without requiring UE modifications.
...
\begin{thebibliography}{00}
	...
	J.~Chen, W.~J.~Hillery, K.~R.~Mestav, C.~Nuzman, I.~Saniee, and Y.~Xing, 
	``CSI Compression for Massive MIMO: Model-Based or Data-Driven?,'' 
	\emph{IEEE Wireless Communications}, vol.~32, no.~1, pp.~22--27, Feb. 2025, 
	...
	X. Zhang, Z. Lu, R. Zeng, and J. Wang,
	``Quantization Adaptor for Bit-Level Deep Learning-Based Massive MIMO CSI Feedback,''
	arXiv preprint arXiv:2211.02937v2, 2022.

	\bibitem{liu2023continuous} 
	Y. Liu et al., 
	``Continuous Online Learning-Based CSI Feedback in Massive MIMO Systems,'' 
	\textit{IEEE Wireless Commun. Lett.}, 
	early access, 2023, doi: 10.1109/LWC.2023.10381825.

	\bibitem{wang2024better} 
	J. Wang et al., 
	``Toward Better Low-Rate DL-Based CSI Feedback: A Test Channel-Based Approach,'' 
	\textit{IEEE Trans. Wireless Commun.}, 
	vol. 23, no. 5, pp. 3210-3224, May 2024.

	\bibitem{zhang2024zone} 
	Y. Zhang, A. Alkhateeb, 
	``Zone-Specific CSI Feedback for Massive MIMO,'' 
	\textit{IEEE Wireless Commun. Lett.}, 
	early access, 2024.
