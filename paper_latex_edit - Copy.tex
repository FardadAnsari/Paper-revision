...
While deep learning methods outperform traditional compressive sensing techniques, their large parameter count poses challenges for deployment on resource-limited devices, such as user equipment. 
...
Where \begin{math}\gamma \end{math}, \begin{math}\ N_{s} \end{math}, \begin{math}\ N_{i} \end{math} and \begin{math}\ N_{t} \end{math} are compression ratio, codeword length, selected rows from \begin{math}\tilde{H} \end{math} and number of transmit antenna.\(N_s\) is the real-valued codeword length, and \(2 N_i N_t\) accounts for the real/imaginary parts of \(\tilde{\mathbf{H}}_f\).
